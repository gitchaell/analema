...
import type { LocationId } from '../src/domain/entities/Types';

function generateUID(date: string, time: string, type: string, locationId: LocationId): string {
	return `${date}-${time.replace(':', '')}-${type}-${locationId}@analemma`;
}
...
async function generateICS(): Promise<string> {
	const icsContent = [
		'BEGIN:VCALENDAR',
		'VERSION:2.0',
		'PRODID:-//Analemma Sky Photographer//EN',
		'CALSCALE:GREGORIAN',
		'METHOD:PUBLISH',
		'X-WR-CALNAME:Analemma Captures 2026',
